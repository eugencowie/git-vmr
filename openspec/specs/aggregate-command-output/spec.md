...
Fan-out Git command failure rendering SHALL avoid duplicate fatal prefixes when Git's selected failure message already begins with `fatal:`.

#### Scenario: Git fatal message is not double-prefixed
- **WHEN** a fan-out command fails in child repositories `alpha` and `zeta`
