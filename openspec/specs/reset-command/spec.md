...
## Purpose
Define `git vmr reset [--soft | --mixed | --hard | --merge | --keep] [<commit>]` behavior for resetting across immediate child repositories in a virtual monorepo, including reset mode handling, Git-delegated repository-state resolution, best-effort execution, deterministic reporting, and existing VMR working directory handling.
