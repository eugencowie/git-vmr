## Purpose
Define how commands discover the virtual monorepo root from a starting directory.

## Requirements

### Requirement: VMR root discovery via ancestor traversal
The system SHALL provide a function that, given a starting directory path, traverses upward through parent directories looking for a `.gitvmr` marker. The marker MAY be a directory or file. The function SHALL return the parent directory of the `.gitvmr` marker as the VMR root.
