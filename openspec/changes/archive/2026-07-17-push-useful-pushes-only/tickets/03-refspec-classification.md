...
- [x] Each repo pushes only its useful refspec subset, preserving user order; empty subset means no invocation
- [x] Delete refspecs always push; tag refspecs always push without a usefulness check
- [x] Wildcard refspecs skip with a reason naming the refspec as unclassifiable
- [x] A URL repository argument passes the whole push through unfiltered
- [x] Mixed-refspec skips are reported per refspec alongside the repo's push result
- [x] Integration tests cover the classification and mixed-refspec scenarios from the delta spec; verbatim-delegation scenarios are rewritten
- [x] Full test suite and lints pass via mise tasks
