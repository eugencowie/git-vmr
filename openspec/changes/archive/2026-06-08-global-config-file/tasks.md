...
- [ ] 1.4 Implement global config path resolution using `GITVMR_CONFIG_DIR` first, then the platform config directory.
...
- [ ] 3.2 Set `GITVMR_CONFIG_DIR` to an isolated temp directory for every integration test command.
...
- [ ] 4.1 Add tests for global config path resolution with and without `GITVMR_CONFIG_DIR`.
