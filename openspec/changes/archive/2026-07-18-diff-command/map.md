...
- [Pager shell resolution and the argv seam](tickets/12-pager-shell-resolution.md) — implemented in 83b8e8d: `Rendered.pager` is an argv, shell resolved via `git var GIT_SHELL_PATH` (literal `sh` fallback), emit prepends `dirname(argv[0])` to the pager's PATH, spawn failure falls back to silent tested direct print (status backfilled while resolving ticket 14).
- [Windows CI test job](tickets/14-windows-ci.md) — `test-windows` job added to `.github/workflows/ci.yml` running `cargo test` on `windows-latest`; caveat recorded in the workflow that the runner has `sh` on PATH, so it proves the honest-breakage class, not the degradation paths (those stay unit-covered by ticket 12's platform-neutral tests).
