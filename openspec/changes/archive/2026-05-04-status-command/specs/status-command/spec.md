...
Repositories in detached HEAD state SHALL each appear in their own section with the header `HEAD detached at <short-hash> (<repo-name>)`.
...
File entry lines SHALL be colored using `anstyle`: green for staged changes, red for unstaged changes, and red for untracked files. Branch headers, section headers, and status summary lines SHALL NOT be colored. Colors SHALL be automatically disabled when output is not a tty (e.g., piped to another command).
