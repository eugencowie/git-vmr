...
The project documentation SHALL disclose that usage analytics are enabled by default before the `1.0.0` release when no explicit analytics configuration is present. The documentation SHALL show how to disable analytics and SHALL list all app-authored properties and known `aptabase-rs` SDK-authored fields sent with events.
