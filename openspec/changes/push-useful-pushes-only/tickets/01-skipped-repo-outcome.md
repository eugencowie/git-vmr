...
- [ ] Repo outcome has a skipped variant with a required reason, alongside success and failure
- [ ] Result aggregation groups skips by identical reason with the repo-list suffix, on stdout
- [ ] Skip lines render dimmed through the existing rendering choke point
- [ ] Skips never affect the exit code; an all-skipped run exits 0 and prints its skip reasons
- [ ] Unit tests cover the aggregate-command-output delta scenarios (grouping, exit code, all-skipped not silent)
