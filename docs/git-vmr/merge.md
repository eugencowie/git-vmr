...
This command is used by **git** **pull** to incorporate changes from
another repository and can be used by hand to merge changes from one
branch into another.
...
message from the user describing the changes. Before the operation,
...
reconstruct the pre-merge state. However, if there were uncommitted
changes when the merge started (and especially if those changes were
...
| ------ | ----------- | ---------- |
| **--commit**, **--no-commit** | Perform the merge and commit the result. This option can be used to override **--no-commit**. | ❌ |
...
| **--ff**, **--no-ff**, **--ff-only** | Specifies how a merge is handled when the merged-in history is already a descendant of the current history. **--ff** is the default unless merging an annotated (and possibly signed) tag that is not stored in its natural place in the **refs/tags/** hierarchy, in which case **--no-ff** is assumed. | ❌ |
...
| **--verify**, **--no-verify** | By default, the pre-merge and commit-msg hooks are run. When **--no-verify** is given, these are bypassed. See also **githooks**(5). | ❌ |
...
| **--progress**, **--no-progress** | Turn progress on/off explicitly. If neither is specified, progress is shown if standard error is connected to a terminal. Note that not all merge strategies may support progress reporting. | ❌ |
...
| **--into-name** *\<branch\>* | Prepare the default merge message as if merging to the branch *\<branch\>*, instead of the name of the real branch to which the merge is made. | ❌ |
...
| *\<commit\>*... | Commits, usually other branch heads, to merge into our branch. Specifying more than one commit will create a merge with more than two parents (affectionately called an Octopus merge). | ✅ (single commit only) |
...
changes overlap with files that **git** **pull**/**git** **merge** may
need to update.
...
exceptions to this rule may exist depending on which merge strategy is
...
is the most common case especially when invoked from **git** **pull**:
...
> 3\.
>
> Paths that merged cleanly are updated both in the index file and in
> your working tree.
...
> with **git** **ls-files** **-u**). The working tree files contain the
> result of the merge operation; i.e. 3-way merge results with familiar
> conflict markers **\<\<\<** **===** **\>\>\>**.
...
**git** **merge**, or pass **--ff-only** when you do not have any work
on your own. e.g.
...
markers **\<\<\<\<\<\<\<**, **=======**, and **\>\>\>\>\>\>\>**. The
part before the **=======** is typically your side, and the part
...
**\>\>\>\>\>\>\>** markers, it uses another **\|\|\|\|\|\|\|** marker
that is followed by the original text. You can tell that the original
just stated a fact, and your side simply gave in to that statement and
gave up, while the other side tried to have a more positive attitude.
You can sometimes come up with a better resolution by viewing the
original.
...
> working tree changes made by 2. and 3.; **git** **merge** **--abort**
...
> Use **git** **commit** or **git** **merge** **--continue** to seal the
> deal. The latter command checks whether there is a (interrupted) merge
> in progress before calling **git** **commit**.
...
> which will work through the merge with you.

> ·
>
> Look at the diffs. **git** **diff** will show a three-way diff,
> highlighting changes from both the **HEAD** and **MERGE_HEAD**
> versions. **git** **diff** **AUTO_MERGE** will show what changes
> you’ve made so far to resolve textual conflicts.

> ·
>
> Look at the diffs from each branch. **git** **log** **--merge** **-p**
> *\<path\>* will show diffs first for the **HEAD** version and then the
> **MERGE_HEAD** version.
...
> changes into a merge commit. Small fixups like bumping release/version
...
> marker. The "merge" style tends to produce smaller conflict regions
> than diff3, both because of the exclusion of the original text, and
> because when a subset of lines match on the two sides, they are just
> pulled out of the conflict region. Another alternate style, "zdiff3",
> is similar to diff3 but removes matching lines on the two sides from
> the conflict region when those matching lines appear near either the
...
> **branch.***\<current-branch\>***.remote** are consulted, and then
...
> the current branch is fast-forwarded. When set to **false**, this
...
> time to new files added to a directory on one side of history when
> that directory was renamed on the other side of history. Possible
> values are:
>
> **false**
>
> > Directory rename detection is disabled, meaning that such new files
> > will be left behind in the old directory.
>
> **true**
>
> > Directory rename detection is enabled, meaning that such new files
...
>
> If **merge.renames** is **false**, **merge.directoryRenames** is
...
> repository, for each file where a three-way content merge is needed,
> Git can convert the data recorded in commits to a canonical form
> before performing a merge to reduce unnecessary conflicts. For more
...
> with care: the final stash application after a successful merge might
...
> built-in values. Any other value is treated as a custom merge tool and
> requires that a corresponding **mergetool.***\<guitool\>***.cmd**
> variable is defined.
>
> **araxis**
>
...
> > **git** **help** **mergetool**s **BACKEND** **SPECIFIC** **HINTS**
> > section)
>
> **gvimdiff1**
>
> > Use gVim (requires a graphical session) with a 2 panes layout (LOCAL
> > and REMOTE)
>
> **gvimdiff2**
>
...
> **gvimdiff3**
>
> > Use gVim (requires a graphical session) where only the MERGED file
> > is shown
>
> **kdiff3**
>
...
> > **merge** (see **git** **help** **mergetool**s **CONFIGURATION**
> > section)
>
> **nvimdiff**
>
...
> is level 2. Can be overridden by the **GIT_MERGE_VERBOSITY**
