...
| **-W**, **--worktree**, **-S**, **--staged** | Specify the restore location. If neither option is specified, by default the working tree is restored. Specifying **--staged** will only restore the index. Specifying both restores both. | ✅ (`--worktree`, `--staged` only) |
...
| **--ours**, **--theirs** | When restoring files in the working tree from the index, use stage \#2 (**ours**) or \#3 (**theirs**) for unmerged paths. This option cannot be used when checking out paths from a tree-ish (i.e. with the **--source** option). | ❌ |
| **-m**, **--merge** | When restoring files on the working tree from the index, recreate the conflicted merge in the unmerged paths. This option cannot be used when checking out paths from a tree-ish (i.e. with the **--source** option). | ❌ |
...
| **--ignore-skip-worktree-bits** | In sparse checkout mode, the default is to only update entries matched by *\<pathspec\>* and sparse patterns in **\$GIT_DIR/info/sparse-checkout**. This option ignores the sparse patterns and unconditionally restores any files in *\<pathspec\>*. | ❌ |
...
| **--overlay**, **--no-overlay** | In overlay mode, never remove files when restoring. In no-overlay mode, remove tracked files that do not appear in the *\<tree\>* of **--source=***\<tree\>*, to make them match *\<tree\>* exactly. The default is no-overlay mode. | ❌ |
...
file globbing is used to match entries in the index (not in the working
tree by the shell).
...
This documentation is derived from the Git man page for `git-restore`.
